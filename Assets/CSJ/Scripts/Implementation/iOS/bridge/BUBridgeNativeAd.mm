...
@interface BUBridgeNativeAd : NSObject
@end

@interface BUBridgeNativeAd () <BUNativeAdDelegate, BUVideoAdViewDelegate,BUAdObjectProtocol>
...
@implementation BUBridgeNativeAd

- (void)dealloc {
    [self dispose];
}

+ (BUBridgeNativeAd *)sharedInstance {
    static BUBridgeNativeAd *manager = nil;
...
void UnionPlatform_NativeAd_Load(
    ...
    NSLog(@"%s",slot->slotId);
    BUBridgeNativeAd* instance = [BUBridgeNativeAd sharedInstance];
...
void UnionPlatform_NativeAd_SetInteractionListener (
   void* nativeAdPtr,
    ...
    int context) {
    BUBridgeNativeAd* nativeAdP = (__bridge BUBridgeNativeAd*)nativeAdPtr;
...
void  UnionPlatform_NativeAd_Dispose(void* nativeAdPtr) {
    BUBridgeNativeAd* nativeAdP = (__bridge BUBridgeNativeAd*)nativeAdPtr;
    [nativeAdP dispose];
    (__bridge_transfer BUBridgeNativeAd*)nativeAdPtr;
}
